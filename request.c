    ...
    if ((http_url(&r, buf)) == 0) {
        ...
        if (r.type == CONNECT) {
            ...
            if (read_header(cl, &r) != 0) {
                ...
                switch (i) {
                case E_INV:
                    info("invalid CONNECT request for (%s:%d) from (%s) [%s]",
                         r.host, r.port, clinfo->name, clinfo->ip);
                    break;
                case E_RES:
                    info("resolve failure for host (%s) from (%s) [%s]",
                         r.host, clinfo->name, clinfo->ip);
                    break;
                case E_CON:
                    info("connection failure for host (%s) from (%s) [%s]",
                         r.host, clinfo->name, clinfo->ip);
                    break;
                default:
                    i = 0;
            ...
            info("filtered request for URL (%s) from (%s) [%s]", r.url,
                 clinfo->name, clinfo->ip);
            if (r.loop)
                warn("LOOP DETECTED for URL (%s) from (%s) [%s]", r.url,
                     clinfo->name, clinfo->ip);
            else
                err_msg(cl, &r, E_FIL);
            ...
            if (config.logrequests)
                info("request for URL (%s) from (%s) [%s]", r.url,
                     clinfo->name, clinfo->ip);

            i = do_request(cl, &r);
            switch (i) {
            case E_INV:
                info("invalid request for URL (%s) from (%s) [%s]", r.url,
                     clinfo->name, clinfo->ip);
                break;
            case E_RES:
                info("resolve failure for host (%s) from (%s) [%s]", r.host,
                     clinfo->name, clinfo->ip);
                break;
            case E_CON:
                info("connection failure for host (%s) from (%s) [%s]", r.host,
                     clinfo->name, clinfo->ip);
                break;
            case E_POST:
                info("failure while post for URL (%s) from (%s) [%s]", r.url,
                     clinfo->name, clinfo->ip);
                break;
            case E_FIL:
                info("filtered request for URL (%s) from (%s) [%s]", r.url,
...
static int read_header(int cl, struct req *r)
{
    size_t len, i;
    char buf[2048];
    char *b, *p;
    ...
    i = 0;
    while ((len = _getline(cl, buf, sizeof(buf))) > 0
           && i < sizeof(r->header) - 1) {
        b = buf;
...
static size_t _getline(int s, char buf[], int len)
{
    int c;
    size_t i;
...
static int do_request(int cl, struct req *r)
{
    extern struct cfg config;
    unsigned long ip;
    int s;
    ...
    if (config.use_ipv6
        && (config.aux_proxy_ipv6 || *config.proxyhost == '\0')) {
        ...
        if (*config.proxyhost != '\0' && config.proxyport) {
            ...
            if (getaddrinfo(r->host, port, &hints, &res)) {
                DEBUG(("do_request() => getaddrinfo() failed for %s",
                       r->host));
                return E_RES;
            }
        }
        ...
        if (*config.proxyhost != '\0' && config.proxyport) {
            ...
            if ((addr.sin_addr.s_addr =
                 resolve(config.proxyhost)) == INADDR_NONE) {
                DEBUG(("do_request() => resolve failure for proxy %s",
                       config.proxyhost));
                return E_RES;
            }
        ...
        if ((s = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
            DEBUG(("do_request() => socket() failed for %s port %d", r->host,
                   r->port));
            return E_CON;
        } else if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &foo, sizeof(foo))
                   != 0) {
            DEBUG(("do_request() => setsockopt() failed for %s port %d",
                   r->host, r->port));
            return E_CON;
        } else if (connect(s, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
            DEBUG(("do_request() => connect() failed for %s port %d", r->host,
                   r->port));
            return E_CON;
        }
    }
    ...
    if (config.accel && config.accelusrhost)
        len = snprintf(buf, sizeof(buf),
                       ...
                       (*config.proxyhost
                        && config.proxyport) != '\0' ? r->url : r->urlpath,
                       r->vmajor, r->vminor);
    else if (r->port == 80)
        len = snprintf(buf, sizeof(buf),
                       "%s %s HTTP/%d.%d\r\n"
        ...
        *buf = '\0';
        ...
        len = snprintf(buf, sizeof(buf),
                       ...
                       (*config.proxyhost
                        && config.proxyport) != '\0' ? r->url : r->urlpath,
                       r->vmajor, r->vminor, r->host, r->port);
    ...
    if (r->type != CONNECT) {
        ...
        DEBUG(("do_request() => request ready: type %d url (%s) host (%s) port %d", r->type, r->url, r->host, r->port));
    ...
    if (r->type != CONNECT) {
        i = 0;
        while ((len = _getline(s, buf, sizeof(buf))) > 0
               && i < sizeof(r->header) - 1) {
